...
df = pd.read_csv('../extract-pdfs/ISINs.csv')

for fund_number, fund_id in enumerate(df['Fund IDs']):
    # Save file
    csv_file = fund_id + '.csv'
    print('Analysing ' + str(fund_id))
    ISIN = df[df['Fund IDs'] == fund_id].iloc[0]['ISINs']
        
    pages = layout_scanner.get_pages('../extract-pdfs/pdf_downloads/' + str(fund_id) + '.pdf')
    dfs = []
    table_pages = []
